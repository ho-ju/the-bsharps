app.controller("statsTotalsController", ["$scope", "$http", "$filter", "apiService", "$timeout", "$interval", "$rootScope", "$window", "$location",
    function($scope, $http, $filter, apiService, $timeout, $interval, $rootScope, $window, $location) {
        ...
        $scope.getStats = function(league, season, callback) {
            ...
            apiService.stats.getTotals($scope.selectedLeague, $scope.selectedSeason, function(data) {
                $timeout(function(){
        ...
        $scope.getStatsFinals = function(callback) {
            $scope.success = false;
            apiService.stats.getTotalsFinals(function(data) {
                $timeout(function(){
                    $scope.statsCallback(data);
                },0);
            }, function(){ $scope.apiError = true; });
        ...
        $scope.getStatsCareer = function(callback) {
            $scope.success = false;
            apiService.stats.getTotalsCareer(function(data) {
                $timeout(function(){
                    $scope.statsCallback(data);
                },0);
            }, function(){ $scope.apiError = true; });
