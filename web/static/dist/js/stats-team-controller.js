app.controller("statsTeamController", ["$scope", "$http", "$filter", "apiService", "$timeout", "$interval", "$rootScope", "$window", "$location",
    function($scope, $http, $filter, apiService, $timeout, $interval, $rootScope, $window, $location) {
        ...
        $scope.sortTypeL     = 'round'; // set the default sort type
        $scope.sortReverseL  = true;  // set the default sort order
        ...
        $scope.getStats = function(type, section) {
            $scope.statType = type || '';
            ...
            $scope.careerTotalsFinals = [];
            $scope.latestSeason = [];
            $scope.latestSeasonTotals = [];

            $timeout(function(){
                if(!$scope.statType) {
                    apiService.stats.getTeam(function(data) {
        ...
        $scope.playerCallback = function(data) {
            if(data && typeof data === 'object') {
                ...
                $scope.careerHighsPTS = data.playersCareerHighPTS[0];
                $scope.careerHighsFTA = data.playersCareerHighFTA[0];
                $scope.careerHighsFTM = data.playersCareerHighFTM[0];
                $scope.careerHighs3PT = data.playersCareerHigh3PT[0];
                $scope.careerHighsFouls = data.playersCareerHighFouls[0];
                $scope.careerHighsFTP = data.playersCareerHighFTP[0];
                $scope.careerHighsFGM = data.playersCareerHighFGM[0];
                $scope.latestSeason = data.latest;
                $scope.latestSeasonTotals = data.latestTot[0];
                $scope.setWinningest();

                $scope.initScoreChart();

                $timeout(function(){
                    $scope.setHoneyFirstly();
                    $scope.setHoneyFirst();
        ...
        $scope.setWinningest = function() {
            $scope.winningestPlayerData = parseFloat((($scope.stats.winningestT[0].winningest / $scope.stats.playerDetail[0].gmsPlayed)*100).toFixed(1));
        };
        ...
        $scope.scrollToEl = function($event) {
            var scrollPos = event.pageY - 30;

            // ReCalc pos of tables
            $timeout(function(){
                if(scrollPos) {
        ...
        $scope.setHoneyFirstly = function() {
            var tableOffset = jQuery("#latest-season-table").offset().top + jQuery("#latest-season-table thead").outerHeight(),
                $header     = jQuery("#latest-season-table thead").clone(),
                $fixedHeader = jQuery(".header-fixed.latest"),
                secondTableOffset = jQuery("#career-totals-table").offset().top - 100,
                headerIsHidden = true;

            $fixedHeader.find('table').empty().prepend($header);
